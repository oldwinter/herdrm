...
grep -Fq ".iOS(.v${EXPECTED_MAJOR})" "${PACKAGE_DIR}/Package.swift" || {
    echo "error: HerdrSSH Package.swift does not target iOS ${EXPECTED_MAJOR}" >&2
    exit 1
}
...
echo "HerdrSSH artifact checksums, slices, privacy manifest, signature, provenance, and algorithm policy are valid."
